...
<template>
  <div class="flex flex-col h-[calc(100vh-64px)] bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-200 overflow-hidden font-sans">
      
      <!-- Toolbar -->
      <div class="bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700 px-4 py-2 flex items-center justify-between shadow-sm z-20 h-14 shrink-0">
          <div class="flex items-center gap-3">
              <h1 class="text-xl font-bold hidden sm:block">粵語探針</h1>
              <div class="text-sm bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded text-slate-500 dark:text-slate-300">
          ...
          <div class="flex items-center gap-2">
              ...
              >
              <button @click="resetAll" class="px-3 py-1.5 text-sm font-medium text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-md">
                  重置
              </button>
          </div>
      </div>
      ...
      <div class="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
          ...
          <div class="flex-1 flex flex-col min-w-0 h-full overflow-hidden bg-slate-100/50 dark:bg-slate-900/50">
                       ...
                       <template v-for="(name, index) in CHARACTERISTIC_PAIRS" :key="index">
                           ...
                           >
                               ...
                               <div class="grid grid-cols-3 gap-1">
                                   <button @click="handleOptionChange(index, 1)" class="h-6 rounded text-xs flex items-center justify-center transition-colors" :class="userVector[index] === 1 ? 'bg-green-600 text-white' : 'bg-slate-100 dark:bg-slate-700 text-slate-400 hover:bg-green-100 dark:hover:bg-green-900 hover:text-green-600'">
                                       ✓
                                   </button>
                                   <button @click="handleOptionChange(index, -1)" class="h-6 rounded text-xs flex items-center justify-center transition-colors" :class="userVector[index] === -1 ? 'bg-red-500 text-white' : 'bg-slate-100 dark:bg-slate-700 text-slate-400 hover:bg-red-100 dark:hover:bg-red-900 hover:text-red-600'">
                                       ✗
                                   </button>
                                   <button @click="handleOptionChange(index, 0)" class="h-6 rounded text-xs flex items-center justify-center transition-colors" :class="userVector[index] === 0 ? 'bg-slate-200 dark:bg-slate-600 text-slate-600 dark:text-slate-300' : 'bg-slate-50 dark:bg-slate-700 text-slate-300 hover:bg-slate-200'">
                                       -
                                   </button>
                               </div>
                           </div>
              ...
              <!-- Bottom Info Panel -->
              <div class="shrink-0 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 shadow-lg z-10 p-4 transition-all duration-300">
                  <div class="flex justify-between items-center cursor-pointer mb-2" @click="isInfoExpanded = !isInfoExpanded">
                       <span class="text-sm font-semibold flex items-center gap-2">
                           關於本頁
                           <span v-if="!isInfoExpanded && topRankName" class="text-xs px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-700 font-normal">
                               最接近: {{ topRankName }}
                           </span>
                       </span>
                  ...
                  <div v-show="isInfoExpanded" class="space-y-3 animate-fade-in">
                       <div class="flex items-center gap-3">
                           <div class="w-6 h-6 rounded-full shadow-sm ring-2 ring-white dark:ring-slate-700" :style="{ backgroundColor: mixedColor }"></div>
          ...
          <!-- Rankings Sidebar -->
          <div class="w-full lg:w-80 h-48 lg:h-auto border-t lg:border-t-0 lg:border-l border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 flex flex-col shrink-0 z-10">
               <div class="p-3 bg-slate-50 dark:bg-slate-700 border-b border-slate-100 dark:border-slate-600 flex justify-between items-center">
               ...
               <div class="flex-1 overflow-y-auto">
                   <template v-for="(item, rank) in rankings" :key="item.id">
                       <div @click="loadDialectData(item.id, item.name)" class="relative px-3 py-2 border-b border-slate-50 dark:border-slate-700 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700 group overflow-hidden">
                            <!-- Bar Background -->
                            <div class="absolute inset-0 opacity-10 pointer-events-none transition-all duration-500 ease-out" 
                                 :style="{ width: item.similarity + '%', backgroundColor: getBarColor(item.name) }"></div>
                            
                            <div class="relative z-10 flex justify-between items-center">
                                <div class="flex items-center gap-2 overflow-hidden">
                                     <span class="w-5 h-5 flex items-center justify-center rounded text-[10px] font-bold shrink-0"
                                ...
                                <span class="text-sm font-bold" :style="{ color: item.similarity > 50 ? getBarColor(item.name) : '#94a3b8' }">
