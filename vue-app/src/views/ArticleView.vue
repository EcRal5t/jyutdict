  ...
  <div class="flex flex-col md:flex-row min-h-screen pt-16 container mx-auto px-4 gap-8">
    ...
    <main class="flex-1 pb-12">
        <div v-if="currentArticle" class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 md:p-8">
            <h1 class="text-3xl font-serif font-bold text-gray-900 dark:text-gray-100 mb-2">{{ currentArticle.title }}</h1>
            <p v-if="currentArticle.intro" class="text-gray-600 dark:text-gray-400 mb-8 italic border-l-4 border-gray-300 pl-4">
                {{ currentArticle.intro }}
            </p>

            <div v-if="currentArticle.table && currentArticle.table.rows" class="overflow-x-auto">
                <table class="w-full border-collapse border border-stone-400 dark:border-stone-600 font-serif text-center">
                    <caption class="font-bold text-lg mb-2 text-stone-700 dark:text-stone-300">{{ currentArticle.table.caption }}</caption>
                    <thead>
                        <tr class="bg-stone-100 dark:bg-stone-900 border-b-2 border-stone-400 dark:border-stone-600">
                            <th v-for="(header, index) in currentArticle.table.headers" :key="index" class="p-2 font-bold text-stone-800 dark:text-stone-200 border-r border-stone-300 dark:border-stone-700 last:border-0">
                                {{ header }}
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="(row, rowIndex) in currentArticle.table.rows" :key="rowIndex" class="hover:bg-amber-50 dark:hover:bg-stone-800 border-b border-stone-300 dark:border-stone-700 text-stone-900 dark:text-stone-100">
                             ...
                             <td class="p-2 border-r border-stone-300 dark:border-stone-700" :class="{'bg-blue-100 dark:bg-blue-900': row.initial === 'n', 'bg-green-100 dark:bg-green-900': row.initial === 'l'}">{{ row.initial }}</td>
                             ...
                             <td class="p-2 border-r border-stone-300 dark:border-stone-700 text-xl" :class="{'italic': row.italic}">
