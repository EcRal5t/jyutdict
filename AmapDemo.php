...
<head><meta http-equiv="Content-Type" content="text/html; charset=utf-16">
    ...
    <?PHP
    	class Mapdata
    	{
			...
			function Faatjam($jam)
			{
				$this->pronunciation = $jam;
			}
			function Mapshow()
			{
				...
				echo 'offset: new AMap.Pixel(20, 20),'."\n";
				echo 'content:"<div class=\'info\'>'.$this->pronunciation.'</div>"'."\n".'});'."\n";
			}
    	}
    	?>
</head>
...
<div class="container">
                ...
                <table style="width: 100%;">
                    <tr>
                        <td width="50%">
                            ...
                            <?PHP	#Zenam 2018-10-19 增加了用于给地图提供数据
	                        if (is_array($resultFanwan)) {
	                        	$fanwanmap =new Mapdata;
	                        	$fanwanmap->Locate("[114.287836,22.483897]");	#地图坐标必须
								$fanwanmap->name("fanwan");						#名字必须用来输出JS用
	                            do {
	                             ...
	                             //其實下面部分想用Collapse
	                                ...
	                                $fanwanmap->Faatjam($resultFanwan[9].$resultFanwan[10].$resultFanwan[11]); #存储发音
	                            } while($resultFanwan = mysqli_fetch_row($queryFanwan));
	                        } else {
	                            echo '耖毋到';
	                        }
                            ?>
                        ...
                        <td width="50%">
                            ...
                            <?PHP
                            	$JingwaaFlag = TRUE;								#用来标记是否第一次搜索处理多音字的
                            	$JingwaaMap = new Mapdata;			
                            	$JingwaaMap->Locate("[114.019186,22.583787]");		#地图坐标必须
								$JingwaaMap->name("Jingwaa");						#名字必须用来输出JS用
                            if (is_array($resultJingwaa)) {
                            	...
                            	if($JingwaaFlag){
	                                ...
	                                if ($resultJingwaa[12]==0)
									{
	                                    ...
	                                    $JingwaaMap->Faatjam($resultJingwaa[8]);		#存储发音
									}
	                                else{
								...
								$JingwaaFlag = FALSE;
								}while($resultJingwaa = mysqli_fetch_row($queryJingwaa));
                            } else {
                                echo '耖毋到';
                            }
                            ?>
    ...
    <div style="flex: 5;">
        <div class="box">
         ...
         -->
			<div id="Mapcontainer" style="height: 600px;width: 500px;"></div> 
